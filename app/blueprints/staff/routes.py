...
def _filtered_query():
    """Build the Staff query honouring the current filter query-string."""
...
def export_csv():
    ...
    rows = [
        [
            ...
            s.hire_date.strftime("%Y-%m-%d") if s.hire_date else "",
        ]
        for s in query.order_by(Staff.name).all()
    ]
    log_action("export", "staff", detail=f"{len(rows)} rows")
    db.session.commit()
    header = [
        "الاسم",
        "الرقم الوظيفي",
    ...
    return csv_response("staff.csv", header, rows)
